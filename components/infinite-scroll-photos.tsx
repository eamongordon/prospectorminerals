...
export default function InfiniteScrollPhotos({
  ...
  const [page, setPage] = useState(initialPhotos ? initialPhotos[initialPhotos.length - 1].number : 0)
  const [ref, inView] = useInView()

  async function loadMorePhotos() {
    const photosQuery = await fetchPhotos({filterObj: {name: search}, cursor: page, limit: 10});
    if (photosQuery.results?.length) {
      setPage(photosQuery.next ? photosQuery.results[photosQuery.results.length -1].number : page)
